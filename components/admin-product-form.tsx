...
export function AdminProductForm({ initial, onSubmit, onClose }: Props) {
  ...
  const inputClass = 'rx-input rounded-[18px]'
  const labelClass = 'mb-2 block text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500'

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="relative flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-[32px] border border-white/70 bg-white/85 shadow-[0_28px_80px_-40px_rgba(15,23,42,0.95)] backdrop-blur-xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200/70 px-6 py-5">
          <div>
            <p className="rx-kicker text-teal-700">Admin editor</p>
            <h2 className="mt-2 text-lg font-bold text-slate-900">{initial ? 'Edit Medicine' : 'Add New Medicine'}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
